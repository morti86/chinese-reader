...
impl Rec {
    pub fn new(model: Session, keys: Vec<char>) -> Self {
        Self { model, keys, min_score: REC_MIN }
    }

    pub fn from_file(model_path: impl AsRef<Path>, keys_path: impl AsRef<Path>) -> ReaderResult<Self> {
        ...
        let keys = " ".chars()
            ...
            .chain(" ".chars())
            .collect();
        Ok(Self { model, keys, min_score: REC_MIN })
    }
    ...
    pub fn predict_char_score(&mut self, img: &DynamicImage) -> ReaderResult<Vec<(char, f32)>> {
        let input = Self::preprocess(img)?;
        let output = self.run_model(&input)?;
        Ok(output)
    }

    pub fn predict_str(&mut self, img: &DynamicImage) -> ReaderResult<String> {
    ...
    fn run_model(&mut self, input: &ArrayBase<OwnedRepr<f32>, Dim<[usize; 4]>>) -> ReaderResult<Vec<(char, f32)>>{
        let outputs = self.model.run(inputs!["x" => TensorRef::from_array_view(input)? ])?;

        let output = outputs.iter().next().ok_or(ReaderError::ocr("no out"))?.1;
